              ...
              <tr class="hover:bg-white/5 transition-colors">
                ...
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-3">
                    ...
                    @if (isAdmin) {
                      <button (click)="openTransfer(product)" title="Transfer to Retail" class="text-purple-400 hover:text-purple-300 font-medium transition-colors">
        ...
        <div class="rounded-2xl p-6 w-full max-w-2xl border shadow-2xl relative my-auto"
          ...
          <form [formGroup]="productForm" (ngSubmit)="submitForm()">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                <label class="block text-xs font-medium mb-1 text-gray-400">Base Unit (e.g., Box, Sack) *</label>
                ...
                @for (unitCtrl of units.controls; track i; let i = $index) {
                  <div [formGroupName]="i" class="flex gap-3 items-end bg-black/20 p-3 rounded-xl border border-white/5 relative group">
                    ...
                    <div class="flex-1">
                      ...
                      <input type="text" formControlName="unit_name" placeholder="e.g. Pallet" class="w-full bg-transparent border-b border-white/10 px-1 py-1 text-sm text-white focus:outline-none focus:border-green-400">
                    </div>
                    <div class="flex-1">
                      <label class="block text-[10px] font-medium mb-1 text-gray-500">Qty in Base</label>
                      <input type="number" formControlName="quantity_in_base" placeholder="e.g. 50" class="w-full bg-transparent border-b border-white/10 px-1 py-1 text-sm text-white focus:outline-none focus:border-green-400">
                    </div>
                    <div class="flex-1">
        ...
        <div class="rounded-2xl p-6 w-full max-w-md border shadow-2xl relative my-auto"
          ...
          <div class="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p class="text-xs" style="color: #9ca3af;">Move stock from Wholesale to another store</p>
            </div>
          </div>
          ...
          @if (transferMode === 'branch') {
            <div class="mb-4">
              ...
              <select [(ngModel)]="transferBusinessId" (ngModelChange)="onBranchSelected($event, 'retail')"
          ...
          <!-- Source info -->
          <div class="rounded-xl p-3 mb-4" style="background: rgba(251,191,36,0.08); border: 1px solid rgba(251,191,36,0.15);">
            <div class="text-xs font-semibold mb-1" style="color: #fbbf24;">FROM — WHOLESALE</div>
          ...
          <div class="mb-4">
            <label class="block text-xs font-medium mb-1 text-gray-400">
              {{ transferMode === 'branch' ? 'Destination ' + branchDestType + ' Product' : 'Destination Retail Product' }}
            </label>
...
export class WholesaleInventory implements OnInit, OnDestroy {
  ...
  branchDestType: 'retail' | 'wholesale' = 'retail';
  ...
  sortBy(column: keyof WholesaleProduct) {
  ...
  openForm(product?: WholesaleProduct) {
    this.selectedProduct = product || null;
    this.productForm.reset({ quantity: 0, cost_price: 0, sell_price: 0, low_stock_alert: 50 });
    this.units.clear();
  ...
  submitForm() {
    ...
    const request = this.selectedProduct 
      ? this.inventoryService.updateWholesaleProduct(this.selectedProduct.id, payload)
      : this.inventoryService.createWholesaleProduct(payload);

    request.subscribe({
      next: (res) => {
        this.toastService.success(`Wholesale Product ${this.selectedProduct ? 'updated' : 'created'} successfully`);
        this.loadProducts();
        this.closeForm();
  ...
  openRestock(product: WholesaleProduct) {
  ...
  submitRestock() {
    ...
    this.isSubmitting = true;
    this.inventoryService.restockWholesaleProduct(this.selectedProduct.id, this.restockControl.value || 0).subscribe({
      next: () => {
  ...
  openDelete(product: WholesaleProduct) {
    this.selectedProduct = product;
  ...
  submitDelete() {
    ...
    this.inventoryService.deleteWholesaleProduct(this.selectedProduct.id).subscribe({
      ...
      error: () => {
        this.toastService.error('Failed to delete wholesale product');
        this.closeDelete();
      }
    });
  }
  ...
  openTransfer(product: WholesaleProduct) {
    ...
    this.branchDestType = 'retail';
    this.destProducts = [];
    this.eligibleBusinesses = [];

    // Load internal retail products
    this.inventoryService.getRetailProducts().subscribe({
      next: (res: any) => {
        this.retailProducts = Array.isArray(res) ? res : (res.data || []);
        this.destProducts = this.retailProducts;
        this.cdr.detectChanges();
      },
      error: () => this.toastService.error('Failed to load retail products')
    });

    // Load eligible branches in background
  ...
  setTransferMode(mode: 'internal' | 'branch') {
    ...
    this.transferBusinessId = null;
    this.branchDestType = 'retail';
    this.destProducts = mode === 'internal' ? this.retailProducts : [];
  }
  ...
  submitTransfer() {
    ...
    const toType = this.transferMode === 'branch' ? this.branchDestType : 'retail';
